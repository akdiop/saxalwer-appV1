      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>
              {wo ? 'Orientation sensible' : 'Orientation sensible'}
            </Text>
        ...
        <View style={styles.progressWrap}>
          {[1, 2, 3, 4].map((value) => (
            <View
              key={value}
              style={[styles.progressBar, step >= value && styles.progressBarActive]}
            />
          ))}
        </View>
        ...
        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>
            {wo
              ? 'Bii dafa xool sa xaalis ak sa sutura'
              : 'Ce parcours prend aussi en compte ton contexte social'}
          </Text>
        ...
        {step === 1 ? (
          <View style={styles.section}>
            ...
            <Text style={styles.sectionDesc}>
              {wo
                ? 'Tannal li gëna jege ci li ngay dund'
                : 'Choisis ce qui correspond le mieux à ton ressenti actuel'}
            </Text>
            ...
            <Pressable
              style={[styles.nextBtn, !canStep1 && styles.nextBtnDisabled]}
        ...
        {step === 2 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {wo
                ? 'Ci sa kawale, naka lañuy xool mbir yii ?'
                : 'Autour de toi, comment ces sujets sont-ils perçus ?'}
            </Text>
            <Text style={styles.sectionDesc}>
              {wo
                ? 'Lii dina dimbali nu jox la ay wax yu gëna dal'
                : 'Cela nous aide à adapter le ton, les mots employés et le niveau de discrétion'}
            </Text>
            <View style={styles.grid}>
            ...
            <Pressable
              style={[styles.nextBtn, !canStep2 && styles.nextBtnDisabled]}
        ...
        {step === 3 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {wo
                ? 'Naka la décision ak accès yi di dox ci kër gi ?'
                : 'Dans ton foyer, comment se passent les décisions et l’accès aux soins ?'}
            </Text>
            <Text style={styles.sectionDesc}>
              {wo
                ? 'Lii am na solo ngir xam lu mën a la tax nga yeg ne consultation dafa metti'
                : 'Nous cherchons à comprendre s’il existe des freins liés au foyer, aux déplacements, à l’argent ou aux autorisations'}
            </Text>
            ...
            <View style={styles.topicBlock}>
              <Text style={styles.topicTitle}>
                {wo ? 'Dem ci soins' : 'Déplacement vers les soins'}
              </Text>
              <View style={styles.grid}>
            ...
            <Pressable
              style={[styles.nextBtn, !canStep3 && styles.nextBtnDisabled]}
        ...
        {step === 4 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {wo
                ? 'Lu nga bëgg nu def ngir nga yeg ne am nga sutura ak doole ?'
                : 'De quoi as-tu le plus besoin pour te sentir en sécurité et garder la main ?'}
            </Text>
            <Text style={styles.sectionDesc}>
              {wo
                ? 'Tannal li nga bëgg gëna am ci ndimbal bi'
                : 'Ce choix nous aide à proposer un accompagnement plus respectueux de tes marges de manœuvre'}
            </Text>
            <View style={styles.grid}>
            ...
            <View style={styles.summaryCard}>
              <Text style={styles.summaryTitle}>
                {wo ? 'Njiitu biñuy xool' : 'Première lecture du contexte'}
              </Text>
              ...
              <Text style={styles.summaryLine}>
                {wo ? 'Risku sutura :' : 'Risque de confidentialité :'}{' '}
                {privacyRisk ? (wo ? 'Waaw' : 'Oui') : wo ? 'Déedéet' : 'Non'}
              </Text>
              <Text style={styles.summaryLine}>
                {wo ? 'Wàll yi ñuy topp :' : 'Dimensions suivies :'}{' '}
                {riskDimensions.length > 0
                  ? riskDimensions.join(wo ? ' • ' : ', ')
                  : wo
                    ? 'Amul'
                    : 'Aucune'}
              </Text>
